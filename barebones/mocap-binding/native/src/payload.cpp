// Barebones MoCap Binding - native lane. A mocap binding that binds nothing. No 3ds Max SDK.
...
JsonValue Describe(){
    ...
    if(!said){ said=true; Say("Hello World - Barebones MoCap Binding (C++) is live; this line came from its payload the first time the host asked the binding for its name."); }
    JsonValue o=JsonValue::Object(); o.Set("name",JsonValue::String("Cartridge MoCap Binding")); return o;
}
void PDescribe(MaxMcpPayloadInfo* I){ if(!I) return; I->AbiVersion=MAXMCP_PAYLOAD_ABI; std::strncpy(I->Name,"slot_mcbind",sizeof(I->Name)-1); std::strncpy(I->Version,CARTRIDGE_VERSION_STRING,sizeof(I->Version)-1); std::strncpy(I->Variant,"slot_mcbind_native",sizeof(I->Variant)-1); }
...
void SetSink(MaxMcpPayloadLogSink S,void* C){ GSink=S; GCtx=C; if(S){ const std::string L=std::string("Barebones MoCap Binding (C++) native payload loaded: v")+CARTRIDGE_VERSION_STRING+". This is the load report, not the greeting."; S(C,2,L.c_str()); } }

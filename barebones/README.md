...
type. For the twenty-four types with a shipping slot it **registers, appears where that type appears, and
...
eleven slots above **ship and run**. A `🔵` in *Verified* means the example reports its `init` on load —
...
The eleven above are the core gallery, but **twenty-four slots ship and run** in all. The other
...
- **Has a slot — ships and runs.** The twenty-four above.

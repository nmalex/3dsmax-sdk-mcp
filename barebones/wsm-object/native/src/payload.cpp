// Barebones Space Warp - native lane. A parametric cube: the smallest real geometry object.
...
JsonValue BuildMesh(const JsonValue& Args)
{
    ...
    {
        bSaidHello = true;
        Say(("Hello World - Barebones Space Warp (C++) is live; this line came from its payload on the "
             "first evaluation (BuildMesh), a box gizmo of edge " + std::to_string(Size) + ".").c_str());
    }
...
JsonValue DescribeUi(const JsonValue& Args)
{
    ...
    Title->Text = "Cartridge Space Warp";
...
void PayloadDescribe(MaxMcpPayloadInfo* OutInfo)
{
    ...
    std::strncpy(OutInfo->Name, "slot_wsmobject", sizeof(OutInfo->Name) - 1);
    std::strncpy(OutInfo->Version, CARTRIDGE_VERSION_STRING, sizeof(OutInfo->Version) - 1);
    std::strncpy(OutInfo->Variant, "slot_wsmobject_native", sizeof(OutInfo->Variant) - 1);
}
...
void PayloadSetLogSink(MaxMcpPayloadLogSink Sink, void* Context)
{
    ...
    if (Sink != nullptr)
    {
        const std::string Line =
            std::string("Barebones Space Warp (C++) native payload loaded: v") + CARTRIDGE_VERSION_STRING

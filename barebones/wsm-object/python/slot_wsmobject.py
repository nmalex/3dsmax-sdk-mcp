"""Barebones Space Warp (Python lane) - a parametric cube, the smallest real geometry object.
...
WHICH SLOT HOSTS IT. The geometry slot loads this payload by its fixed module name, `slot_wsmobject`.
In Create > Geometry > MCP Cartridges it is **Cartridge Space Warp**; its internal name is
...
Geometry and read `cartridge_logs -module slot_wsmobject`.
"""
...
def BuildMesh(params=None, **_event):
    ...
    if not _said_hello:
        ...
        mcp_bootstrap.log("display",
                          "Hello World - Barebones Space Warp (Python) is live; this line came from its "
                          "payload on the first evaluation (BuildMesh), a box gizmo of edge %g." % size)
    half = size * 0.5
...
def describe(params=None):
    return {"payload": "slot_wsmobject", "version": "0.1.0", "lane": "python",
            "example": "barebones/wsm-object", "builds": "box gizmo"}

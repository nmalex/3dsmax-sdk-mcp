// Barebones Ref Target - native lane. A plain reference target that holds nothing. No 3ds Max SDK.
...
JsonValue Describe(){
    ...
    if(!said){ said=true; Say("Hello World - Barebones Ref Target (C++) is live; this line came from its payload the first time the host asked the target for its class name."); }
    JsonValue o=JsonValue::Object(); o.Set("name",JsonValue::String("Cartridge Ref Target")); return o;
}
void PDescribe(MaxMcpPayloadInfo* I){ if(!I) return; I->AbiVersion=MAXMCP_PAYLOAD_ABI; std::strncpy(I->Name,"slot_reftarget",sizeof(I->Name)-1); std::strncpy(I->Version,CARTRIDGE_VERSION_STRING,sizeof(I->Version)-1); std::strncpy(I->Variant,"slot_reftarget_native",sizeof(I->Variant)-1); }
...
void SetSink(MaxMcpPayloadLogSink S,void* C){ GSink=S; GCtx=C; if(S){ const std::string L=std::string("Barebones Ref Target (C++) native payload loaded: v")+CARTRIDGE_VERSION_STRING+". This is the load report, not the greeting."; S(C,2,L.c_str()); } }

"""Barebones Texture Output (Python lane) - a texmap sub-component that says hello.

WHAT THIS CARTRIDGE IS. A Texture Output (the output curve/level filter on a texmap). 3ds Max creates it as part of a texmap and,
...
WHICH SLOT HOSTS IT. The payload the `Cartridge Texture Output` slot loads by its fixed module name,
`slot_texoutput`, registering one SuperClassID: TEXOUTPUT_CLASS_ID. See this example's README.md.
...
def _hello_once(where):
    ...
    if not _said_hello:
        ...
        mcp_bootstrap.log("display",
                          "Hello World - Barebones Texture Output (Python) is live; this line came from "
...
def describe(params=None):
    return {"payload": "slot_texoutput", "version": "0.1.0", "lane": "python",
            "example": "barebones/texture-output", "superClassId": "TEXOUTPUT_CLASS_ID"}
...
# -- the panel -----------------------------------------------------------------------------------
#
# Shown in the Material Editor. The slot builds it from describe_ui() and puts its own About (version, build,
...
def describe_ui(params=None, **kwargs):
    ...
    return ui.build(
        ui.VBox(
            ui.Label("Barebones Texture Output"),
            ui.Spacer(),
...
def _panel_hello():
    mcp_bootstrap.log("display",
                      "Hello World - the Barebones Texture Output panel is live and this line came from its "

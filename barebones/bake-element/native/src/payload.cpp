// Barebones Bake Element - native lane. A Render To Texture output that bakes nothing. No 3ds Max SDK.
...
JsonValue Describe(){
    ...
    if(!said){ said=true; Say("Hello World - Barebones Bake Element (C++) is live; this line came from its payload the first time the host asked the bake element for its name."); }
    JsonValue o=JsonValue::Object(); o.Set("name",JsonValue::String("Cartridge Bake Element")); return o;
}
void PDescribe(MaxMcpPayloadInfo* I){ if(!I) return; I->AbiVersion=MAXMCP_PAYLOAD_ABI; std::strncpy(I->Name,"slot_bakeelement",sizeof(I->Name)-1); std::strncpy(I->Version,CARTRIDGE_VERSION_STRING,sizeof(I->Version)-1); std::strncpy(I->Variant,"slot_bakeelement_native",sizeof(I->Variant)-1); }
...
void SetSink(MaxMcpPayloadLogSink S,void* C){ GSink=S; GCtx=C; if(S){ const std::string L=std::string("Barebones Bake Element (C++) native payload loaded: v")+CARTRIDGE_VERSION_STRING+". This is the load report, not the greeting."; S(C,2,L.c_str()); } }

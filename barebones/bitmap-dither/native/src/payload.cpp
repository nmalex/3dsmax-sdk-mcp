// Barebones Bitmap Dither (C++ native lane) - documents the shape - this super-class is not third-party registerable.
//
// A bitmap dithering method - the dither applied when reducing an image's bit depth. SuperClassID BMM_DITHER_CLASS_ID. No slot ships for this type, and none ever will - the super-class is not open to third-party registration (see this example's README for the SDK-checked reason); this is the shape.
...
void PayloadDescribe(MaxMcpPayloadInfo* OutInfo)
{
    ...
    std::strncpy(OutInfo->Name, "slot_bmmdither", sizeof(OutInfo->Name) - 1);
    std::strncpy(OutInfo->Version, CARTRIDGE_VERSION_STRING, sizeof(OutInfo->Version) - 1);
    std::strncpy(OutInfo->Variant, "slot_bmmdither_native", sizeof(OutInfo->Variant) - 1);
}
...
void PayloadSetLogSink(MaxMcpPayloadLogSink Sink, void* Context)
{
    ...
    if (Sink != nullptr)
    {
        const std::string Line =
            std::string("Hello World - Barebones Bitmap Dither (C++) native payload loaded: v")

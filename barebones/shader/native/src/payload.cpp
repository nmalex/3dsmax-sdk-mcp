// Barebones Shader - native lane, UNMANAGED. Wrap lighting plus a rim term, per sample.
//
// WHAT THIS SHOWS. A Standard-material shading model with no 3ds Max header. The material has already
// textured its inputs; ShadeSampleIllumRead reads them by the shader's own channel number (0 is the
// diffuse colour), ShadeSampleLight walks the lights, and the lit colour goes back as floats that the
// slot writes into the material's IllumParams.
//
//   wrap diffuse = light * max(0, (N.L + wrap) / (1 + wrap))     - light that bends past the terminator
//   colour       = diffuse input * tint * (ambient + sum of wrap diffuse) + rim * (1 - facing)^2
//   tint         = hue at saturation strength - the same tint the Python lane hands the slot
//
// Exporting MaxMcpGetUnmanaged is the declaration of unmanaged mode. See CartridgePayloadApi.h.
//
// DEPLOY. Build, then put slot_shader_native.dll beside ShaderSlot.dlb and restart 3ds Max.
...
// The shader's channel numbers (ShaderSlot.cpp publishes the same table).
const int32_t kChannelDiffuse = 0;
...
void PayloadDescribe(MaxMcpPayloadInfo* OutInfo)
{
    ...
    std::strncpy(OutInfo->Name, "slot_shader", sizeof(OutInfo->Name) - 1);
...
void* PayloadCreateInstance(void) { return new Model(); }
void PayloadDestroyInstance(void* Instance) { delete static_cast<Model*>(Instance); }
...
// declaration python/slot_shader.py makes; the panel is composed with PayloadUi.h, the same layout
// engine mcp_ui is.
struct ParamSpec
{
    const char* Name;
...
const ParamSpec kParams[] = {
    { "hue", "float", 0.08, 0.0, 1.0, 0.01, "Hue:" },
    { "strength", "float", 0.7, 0.0, 1.0, 0.01, "Strength:" },
    { "wrap", "float", 0.5, 0.0, 1.0, 0.01, "Wrap:" },
    { "rim", "float", 0.2, 0.0, 1.0, 0.01, "Rim:" },
};

const char* const kPanelTitle = "Tint, wrap and rim";
                                ...
                                char* ReplyJson, size_t ReplyCapacity)
{
    ...
    if (std::strcmp(Function, "ShaderUpdate") != 0)
    {
        JsonValue Result;
    ...
    if (Args.At("event").At("probe").AsNumber(0.0) == 0.0)
    {
        ...
        {
            bSaid = true;
            Say("Hello World - Barebones Shader (C++, unmanaged) is live; this line came from its "
                "payload on the first update (ShaderUpdate), and every sample is now lit here.");
        }
    }
    return WriteReply(Reply, ReplyJson, ReplyCapacity) ? MAXMCP_PAYLOAD_OK
                                                       : MAXMCP_PAYLOAD_REPLY_TOO_LARGE;
...
void PayloadSetLogSink(MaxMcpPayloadLogSink Sink, void* Context)
{
    ...
    if (Sink != nullptr)
    {
        const std::string Line = std::string("Barebones Shader (C++) native payload loaded: v")
...
{
    Model* const Self = static_cast<Model*>(Instance);
    if (Self == nullptr) { return; }
    JsonValue Params;
    if (!ParseJson(ParamsJson != nullptr ? ParamsJson : "{}", Params)) { return; }
    double Strength = Params.At("strength").AsNumber(0.7);
    Strength = Strength < 0.0 ? 0.0 : (Strength > 1.0 ? 1.0 : Strength);
    Hsv(static_cast<float>(Params.At("hue").AsNumber(0.08)), static_cast<float>(Strength), 1.0f,
        Self->Tint);
    Self->Wrap = static_cast<float>(Params.At("wrap").AsNumber(0.5));
    Self->Rim = static_cast<float>(Params.At("rim").AsNumber(0.2));
}

int32_t ShaderIllum(void* Instance, MaxMcpShadeSampleHandle Handle, float* OutRGB,
                    float* OutTransparencyRGB)
{
    const Model* const Self = static_cast<const Model*>(Instance);
    if (Self == nullptr || GFacade == nullptr || GFacade->ShadeSampleRead == nullptr
        || GFacade->ShadeSampleIllumRead == nullptr || GFacade->ShadeSampleLight == nullptr)
    {
        return 1;
    }
    MaxMcpShadeSample S{};
    ...
    MaxMcpColor Cd{};
    if (GFacade->ShadeSampleIllumRead(GFacade->Context, Handle, kChannelDiffuse, &Cd)
        != MAXMCP_FACADE_OK)
    {
        return 1;
    }
    ...
    MaxMcpColor Ambient{};
    if (GFacade->ShadeSampleAmbient != nullptr)
    {
        GFacade->ShadeSampleAmbient(GFacade->Context, Handle, &Ambient);
    }
    float Light[3] = { Ambient.r, Ambient.g, Ambient.b };
    for (int32_t i = 0; i < S.NumLights; ++i)
    {
        ...
        float W = (DotNL + Self->Wrap) / (1.0f + Self->Wrap);
        W = W < 0.0f ? 0.0f : W;
        Light[0] += W * C.r;
        Light[1] += W * C.g;
        Light[2] += W * C.b;
    ...
    const float In[3] = { Cd.r, Cd.g, Cd.b };
    for (int c = 0; c < 3; ++c)
    {
        OutRGB[c] = In[c] * Self->Tint[c] * Light[c] + Edge;

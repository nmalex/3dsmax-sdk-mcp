...
// (cartridge_logs -module slot_manipulator). The type's own functions are left to the default here; the
// Python lane in ../python/slot_manipulator.py shows their shape, and PayloadCall below is where a native
// author serves them.
//
...
{
    // The native lane serves the same named functions the Python lane does (see ../python/slot_manipulator.py).
...
void PayloadSetLogSink(MaxMcpPayloadLogSink Sink, void* Context)
{
    ...
    if (Sink != nullptr)
    {
        const std::string Line =
            std::string("Hello World - Barebones Manipulator (C++) native payload loaded: v")

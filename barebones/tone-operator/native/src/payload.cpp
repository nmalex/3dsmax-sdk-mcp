// Barebones Tone Operator - native lane. A no-op tone operator. No 3ds Max SDK.
...
JsonValue DescribeUi(){
    ...
    if(!said){ said=true; Say("Hello World - Barebones Tone Operator (C++) is live; this line came from its payload when its Exposure Control panel was opened."); }
    namespace ui=maxmcp::payloadui; ui::VBoxWidget R; auto t=std::make_shared<ui::LabelWidget>(); t->Text="Cartridge Tone Operator"; R.Children.push_back(t); R.Children.push_back(std::make_shared<ui::SpacerWidget>()); auto l=std::make_shared<ui::LabelWidget>(); l->Text="A no-op tone operator."; R.Children.push_back(l); return ui::Build(R);
}
...
void PDescribe(MaxMcpPayloadInfo* I){ if(!I) return; I->AbiVersion=MAXMCP_PAYLOAD_ABI; std::strncpy(I->Name,"slot_toneop",sizeof(I->Name)-1); std::strncpy(I->Version,CARTRIDGE_VERSION_STRING,sizeof(I->Version)-1); std::strncpy(I->Variant,"slot_toneop_native",sizeof(I->Variant)-1); }
...
void SetSink(MaxMcpPayloadLogSink S,void* C){ GSink=S; GCtx=C; if(S){ const std::string L=std::string("Barebones Tone Operator (C++) native payload loaded: v")+CARTRIDGE_VERSION_STRING+". This is the load report, not the greeting."; S(C,2,L.c_str()); } }

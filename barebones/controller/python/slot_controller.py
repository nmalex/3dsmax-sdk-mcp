"""Barebones Controller (Python lane) - TEMPLATE, slot pending.

An animation controller (Control base) - assigned to a track; the same shape serves every CTRL_* value type.

This plugin type is in the super-class census (SuperClassID CTRL_FLOAT_CLASS_ID) but no slot ships for it yet, so
...
controller slot ships, wire this hello into the function that slot dispatches, and add your behaviour.
"""
...
def describe(params=None):
    return {"payload": "slot_controller", "version": "0.1.0", "lane": "python",
            "example": "barebones/controller", "template": True,
            "superClassId": "CTRL_FLOAT_CLASS_ID"}

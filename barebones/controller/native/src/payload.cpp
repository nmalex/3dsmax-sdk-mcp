...
void PayloadDescribe(MaxMcpPayloadInfo* OutInfo)
{
    ...
    std::strncpy(OutInfo->Name, "slot_controller", sizeof(OutInfo->Name) - 1);
    std::strncpy(OutInfo->Version, CARTRIDGE_VERSION_STRING, sizeof(OutInfo->Version) - 1);
    std::strncpy(OutInfo->Variant, "slot_controller_native", sizeof(OutInfo->Variant) - 1);
}
...
void PayloadSetLogSink(MaxMcpPayloadLogSink Sink, void* Context)
{
    ...
    if (Sink != nullptr)
    {
        const std::string Line =
            std::string("Hello World - Barebones Controller (C++) native payload loaded: v")

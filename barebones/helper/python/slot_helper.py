"""Barebones Helper (Python lane) - Tetra Helper, a plain helper that draws a wireframe tetrahedron.
...
WHICH SLOT HOSTS IT. The helper slot loads this payload by its fixed module name, `slot_helper`. In
the Create panel (Helpers > MCP Cartridges) and in the scene the helper is called **Tetra Helper**;
...
Create a Tetra Helper and read `cartridge_logs -module slot_helper`.
"""
...
def HelperLines(params=None, **_event):
    ...
    if not _said_hello:
        ...
        mcp_bootstrap.log("display",
                          "Hello World - Barebones Helper (Python) is live; this line came from its "
                          "payload on the first draw (HelperLines), a tetrahedron of edge %g." % size)
...
def describe(params=None):
    return {"payload": "slot_helper", "version": "0.1.0", "lane": "python",
            "example": "barebones/helper", "draws": "tetrahedron"}

// Barebones Helper - native lane. Tetra Helper: a plain helper that draws a wireframe tetrahedron.
...
JsonValue HelperLines(const JsonValue& Args)
{
    ...
    {
        bSaidHello = true;
        Say(("Hello World - Barebones Helper (C++) is live; this line came from its payload on the "
             "first draw (HelperLines), a tetrahedron of edge " + std::to_string(Size) + ".").c_str());
    }
...
void PayloadDescribe(MaxMcpPayloadInfo* OutInfo)
{
    ...
    std::strncpy(OutInfo->Name, "slot_helper", sizeof(OutInfo->Name) - 1);
    std::strncpy(OutInfo->Version, CARTRIDGE_VERSION_STRING, sizeof(OutInfo->Version) - 1);
    std::strncpy(OutInfo->Variant, "slot_helper_native", sizeof(OutInfo->Variant) - 1);
}
...
void PayloadSetLogSink(MaxMcpPayloadLogSink Sink, void* Context)
{
    ...
    if (Sink != nullptr)
    {
        const std::string Line =
            std::string("Barebones Helper (C++) native payload loaded: v") + CARTRIDGE_VERSION_STRING

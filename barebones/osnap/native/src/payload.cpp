// Barebones Object Snap - native lane. A plain DLL the slot loads; 3ds Max never sees it. No 3ds Max SDK.
//
...
// (cartridge_logs -module slot_osnap). The type's own functions are left to the default here; the
// Python lane in ../python/slot_osnap.py shows their shape, and PayloadCall below is where a native
// author serves them.
//
...
void PayloadDescribe(MaxMcpPayloadInfo* OutInfo)
{
    ...
    std::strncpy(OutInfo->Name, "slot_osnap", sizeof(OutInfo->Name) - 1);
    std::strncpy(OutInfo->Version, CARTRIDGE_VERSION_STRING, sizeof(OutInfo->Version) - 1);
    std::strncpy(OutInfo->Variant, "slot_osnap_native", sizeof(OutInfo->Variant) - 1);
}
...
{
    // The native lane serves the same named functions the Python lane does (see ../python/slot_osnap.py).
...
void PayloadSetLogSink(MaxMcpPayloadLogSink Sink, void* Context)
{
    ...
    if (Sink != nullptr)
    {
        const std::string Line =
            std::string("Hello World - Barebones Object Snap (C++) native payload loaded: v")

// Barebones Bake Map Operator (C++ native lane) - documents the shape - this super-class is not third-party registerable.
//
// A Bake To Texture bake-map operator - an operator over a common bake map. SuperClassID COMMON_BAKE_MAP_OPERATOR_CLASS_ID. No slot ships for this type, and none ever will - the super-class is not open to third-party registration (see this example's README for the SDK-checked reason); this is the shape.
...
void PayloadDescribe(MaxMcpPayloadInfo* OutInfo)
{
    ...
    std::strncpy(OutInfo->Name, "slot_bakemapop", sizeof(OutInfo->Name) - 1);
    std::strncpy(OutInfo->Version, CARTRIDGE_VERSION_STRING, sizeof(OutInfo->Version) - 1);
    std::strncpy(OutInfo->Variant, "slot_bakemapop_native", sizeof(OutInfo->Variant) - 1);
}
...
void PayloadSetLogSink(MaxMcpPayloadLogSink Sink, void* Context)
{
    ...
    if (Sink != nullptr)
    {
        const std::string Line =
            std::string("Hello World - Barebones Bake Map Operator (C++) native payload loaded: v")

"""Barebones Data Channel Engine (Python lane) - a Data Channel modifier engine; here, one that just says hello.

WHICH SLOT HOSTS IT. The payload the `Cartridge Data Channel Engine` slot loads by its fixed module name,
`slot_dcengine`, registering one SuperClassID: DATACHANNELENGINE_SUPER_CLASS_ID. See this example's README.md.

THE HELLO. Logged once from `process`, which the slot calls when 3ds Max drives this plugin. Trigger it
as the README describes, then read `cartridge_logs -module slot_dcengine`.
"""
...
def process(**event):
    ...
    _hello_once("channel process (process)")
    return {"ok": True}


def describe(params=None):
    return {"payload": "slot_dcengine", "version": "0.1.0", "lane": "python",
            "example": "barebones/datachannel-engine", "superClassId": "DATACHANNELENGINE_SUPER_CLASS_ID"}
...
# -- the panel -----------------------------------------------------------------------------------
#
# Shown in the Data Channel modifier's engine panel. The slot builds it from describe_ui() and puts its own About (version, build,
...
def describe_ui(params=None, **kwargs):
    ...
    return ui.build(
        ui.VBox(
            ui.Label("Barebones Data Channel"),
            ui.Spacer(),
...
def _panel_hello():
    mcp_bootstrap.log("display",
                      "Hello World - the Barebones Data Channel panel is live and this line came from its "

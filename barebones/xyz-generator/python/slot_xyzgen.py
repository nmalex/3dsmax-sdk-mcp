"""Barebones XYZ Generator (Python lane) - a texmap sub-component that says hello.

WHAT THIS CARTRIDGE IS. A XYZ Generator (3D map input - the object/world XYZ a 3D texmap samples in). 3ds Max creates it as part of a texmap and,
...
WHICH SLOT HOSTS IT. The payload the `Cartridge XYZ Generator` slot loads by its fixed module name,
`slot_xyzgen`, registering one SuperClassID: XYZGEN_CLASS_ID. See this example's README.md.
...
def describe(params=None):
    return {"payload": "slot_xyzgen", "version": "0.1.0", "lane": "python",
            "example": "barebones/xyz-generator", "superClassId": "XYZGEN_CLASS_ID"}
...
def describe_ui(params=None, **kwargs):
    ...
    return ui.build(
        ui.VBox(
            ui.Label("Barebones XYZ Generator"),
            ui.Spacer(),
...
def _panel_hello():
    mcp_bootstrap.log("display",
                      "Hello World - the Barebones XYZ Generator panel is live and this line came from its "

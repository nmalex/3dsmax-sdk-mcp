"""Barebones Color Picker (Python lane) - TEMPLATE, slot pending.

A color picker - the modal color chooser the host opens when a swatch is clicked.

This plugin type is in the super-class census (SuperClassID COLPICK_CLASS_ID) but no slot ships for it yet, so
...
color-picker slot ships, wire this hello into the function that slot dispatches, and add your behaviour.
"""
...
def describe(params=None):
    return {"payload": "slot_colpick", "version": "0.1.0", "lane": "python",
            "example": "barebones/color-picker", "template": True,
            "superClassId": "COLPICK_CLASS_ID"}

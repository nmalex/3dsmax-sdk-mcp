...
the reason); [COVERAGE_MAP.md](COVERAGE_MAP.md) is the per-type accounting of how the 23 are verified.

The 23 that ship:

`modifier` · `exporter` · `renderer` · `manipulator` · `helper` · `controller` · `uv-generator` ·
...
done?** A large share of the 3ds Max plugin surface beyond these 23 is already mechanically proven,
...
(they are `osnap`, `manipulator`, `iksolver`, `effect` and `pfoperator` among the 23), which is the

...
the reason); [COVERAGE_MAP.md](COVERAGE_MAP.md) is the per-type accounting of how the 26 are verified.

The 26 that ship:

`geometry` · `shape` · `light` · `modifier` · `exporter` · `renderer` · `manipulator` · `helper` · `controller` · `uv-generator` ·
...
done?** A large share of the 3ds Max plugin surface beyond these 26 is already mechanically proven,
...
(they are `osnap`, `manipulator`, `iksolver`, `effect` and `pfoperator` among the 26), which is the

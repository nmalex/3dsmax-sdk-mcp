...
the reason); [COVERAGE_MAP.md](COVERAGE_MAP.md) is the per-type accounting of how the 27 are verified.

The 27 that ship:

`geometry` · `shape` · `light` · `camera` · `modifier` · `exporter` · `renderer` · `manipulator` · `helper` · `controller` · `uv-generator` ·
...
done?** A large share of the 3ds Max plugin surface beyond these 27 is already mechanically proven,
...
(they are `osnap`, `manipulator`, `iksolver`, `effect` and `pfoperator` among the 27), which is the

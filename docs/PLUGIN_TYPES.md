...
the reason); [COVERAGE_MAP.md](COVERAGE_MAP.md) is the per-type accounting of how the 24 are verified.

The 24 that ship:

`geometry` · `modifier` · `exporter` · `renderer` · `manipulator` · `helper` · `controller` · `uv-generator` ·
...
done?** A large share of the 3ds Max plugin surface beyond these 24 is already mechanically proven,
...
(they are `osnap`, `manipulator`, `iksolver`, `effect` and `pfoperator` among the 24), which is the

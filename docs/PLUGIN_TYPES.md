...
the reason); [COVERAGE_MAP.md](COVERAGE_MAP.md) is the per-type accounting of how the 30 are verified.

The 30 that ship:

`geometry` · `shape` · `light` · `camera` · `wsm-object` · `system` · `custom-attributes` · `modifier` · `exporter` · `renderer` · `manipulator` · `helper` · `controller` · `uv-generator` ·
...
done?** A large share of the 3ds Max plugin surface beyond these 30 is already mechanically proven,
...
| HelperObject | `HELPER_CLASS_ID` | `object.h` | plain helper *(now ships as the `helper` Slot; manipulator & pfoperator also ship under this id)* |
...
(they are `osnap`, `manipulator`, `iksolver`, `effect` and `pfoperator` among the 30), which is the

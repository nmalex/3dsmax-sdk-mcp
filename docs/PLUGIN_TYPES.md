...
the reason); [COVERAGE_MAP.md](COVERAGE_MAP.md) is the per-type accounting of how the 25 are verified.

The 25 that ship:

`geometry` · `shape` · `modifier` · `exporter` · `renderer` · `manipulator` · `helper` · `controller` · `uv-generator` ·
...
done?** A large share of the 3ds Max plugin surface beyond these 25 is already mechanically proven,
...
(they are `osnap`, `manipulator`, `iksolver`, `effect` and `pfoperator` among the 25), which is the

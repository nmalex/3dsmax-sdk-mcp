...
verification detail behind the 30 that ship, see [COVERAGE_MAP.md](COVERAGE_MAP.md).
...
- **Has a slot — ✅ / 🔵** — a slot ships, so its [`barebones/`](../barebones/) example loads into
  3ds Max and reports itself (read the greeting with `cartridge_logs`). **Thirty** types are here.
...
## Has a slot — the 30 that run today

Each row registers a real plugin under its super-class; the [`barebones/`](../barebones/) example is
the payload it loads. **Every one of the 30 reports itself on load:** the slot calls its cartridge's
...
the slot": a freshly installed host loads every slot and shows all 30 reporting on load; on a later
...
| shape | `SHAPE_CLASS_ID` | [shape](../barebones/shape/) | ✅ greeting driven |
| light | `LIGHT_CLASS_ID` | [light](../barebones/light/) | ✅ greeting driven |
| camera | `CAMERA_CLASS_ID` | [camera](../barebones/camera/) | ✅ greeting driven |
| wsm-object | `WSM_OBJECT_CLASS_ID` | [wsm-object](../barebones/wsm-object/) | ✅ greeting driven |
| system | `SYSTEM_CLASS_ID` | [system](../barebones/system/) | 🔵 alive on load |
| custom-attributes | `CUST_ATTRIB_CLASS_ID` | [custom-attributes](../barebones/custom-attributes/) | ✅ greeting driven |
...
its own super-class ID**. `CTRL_FLOAT_CLASS_ID` is the one that ships as a slot today (in the 30

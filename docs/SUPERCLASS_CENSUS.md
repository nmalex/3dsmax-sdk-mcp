...
verification detail behind the 26 that ship, see [COVERAGE_MAP.md](COVERAGE_MAP.md).
...
- **Has a slot — ✅ / 🔵** — a slot ships, so its [`barebones/`](../barebones/) example loads into
  3ds Max and reports itself (read the greeting with `cartridge_logs`). **Twenty-six** types are here.
...
## Has a slot — the 26 that run today

Each row registers a real plugin under its super-class; the [`barebones/`](../barebones/) example is
the payload it loads. **Every one of the 26 reports itself on load:** the slot calls its cartridge's
...
the slot": a freshly installed host loads every slot and shows all 26 reporting on load; on a later
...
its own super-class ID**. `CTRL_FLOAT_CLASS_ID` is the one that ships as a slot today (in the 26

...
This kit authors a hello‑world barebone for **39** 3ds Max plugin types (the super‑class census, FR‑0002).
Not all 39 receive a runnable slot, and that is a **deliberate, evidence‑based** outcome, not a gap: a
...
**39 = 30 slots + 8 shelved (not third‑party registerable) + 1 deferred (registers, hello unreachable).**

---

## 30 types with a shipped slot

Each registers a real plugin under its super‑class. **Every one of the 30 reports itself on load**: the
...
| system | `SYSTEM_CLASS_ID` | 🔵 alive on load, greeting on Create ▸ Systems |
| custom-attributes | `CUST_ATTRIB_CLASS_ID` | ✅ greeting driven |
